...
The rails are rendered using an `IDrawable` (in the `ThrottleRails` class), the handle is another `IDrawablw` (in the `ThrottleHandle` class), and a `ContentView` (the `Throttle` class) combines them and handle the logic for dragging the handle and a bindable property for the RPM value.
...
If you've ever peaked under the hood of an SVG file (or you know how the format works), it's very similar.
...
![Batmobile final layout](/images/batmobile-final-layout.png)
